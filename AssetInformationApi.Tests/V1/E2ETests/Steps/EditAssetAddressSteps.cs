...
{
    public class EditAssetAddressSteps : BaseSteps
    {
        private readonly IDynamoDBContext _dbContext;

        public EditAssetAddressSteps(HttpClient httpClient, IDynamoDBContext dbContext) : base(httpClient)
        {
        ...
        public async Task WhenEditAssetAddressApiIsCalled(Guid id, object requestObject)
        {
            ...
            await WhenEditAssetAddressApiIsCalled(id, requestObject, defaultIfMatch).ConfigureAwait(false);
        }

        public async Task WhenEditAssetAddressApiIsCalled(Guid id, object requestObject, int? ifMatch)
        {
            var token =
                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMTUwMTgxMTYwOTIwOTg2NzYxMTMiLCJlbWFpbCI6ImUyZS10ZXN0aW5nQGRldmVsb3BtZW50LmNvbSIsImlzcyI6IkhhY2tuZXkiLCJuYW1lIjoiVGVzdGVyIiwiZ3JvdXBzIjpbImUyZS10ZXN0aW5nIl0sImlhdCI6MTYyMzA1ODIzMn0.SooWAr-NUZLwW8brgiGpi2jZdWjyZBwp4GJikn0PvEw";

            // setup request
            var uri = new Uri($"api/v1/assets/{id}/address", UriKind.Relative);
        ...
        public async Task TheAssetHasBeenUpdatedInTheDatabase(AssetsFixture assetFixture)
        {
            ...
            databaseResponse.Id.Should().Be(assetFixture.ExistingAsset.Id);
            databaseResponse.AssetAddress.ToString().Should().Be(assetFixture.EditAssetAddress.AssetAddress.ToString());
        }
